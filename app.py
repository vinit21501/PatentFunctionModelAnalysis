...
def main():
    ...
    **How it works:**
    ...
    if 'final_output' in st.session_state:
        ...
        def parse_matrix(llm_output, max_attempts=3):
            sections = {}
            for attempt in range(max_attempts):
                try:
                    ...
                    # If matrix not found or invalid, try with a different chunk
                    if attempt < max_attempts - 1:
                        st.warning(f"Attempt {attempt + 1}/{max_attempts}: Matrix not found. Trying with different chunk...")
                        # Get a different chunk
                        valid_chunks = st.session_state['valid_chunks']
                        vectors = np.array(st.session_state['vectors'])
                        # Use a different chunk index based on attempt number and previous attempts
                        ...
                        # Generate new summary with this chunk
                        llm = st.session_state['llm']
                        summaries = st.session_state['summaries']
                        
                        # Get previous summaries if they exist
                        previous_summaries = st.session_state.get('previous_summaries', [])
                        
                        # Create new summary using the selected chunk
                        ...
                        # Store the new summary for future attempts
                        ...
                        # Generate final output with the combined summary
                    ...
                    if attempt < max_attempts - 1:
                        ...
                        valid_chunks = st.session_state['valid_chunks']
                        vectors = np.array(st.session_state['vectors'])
                        chunk_idx = (attempt + 1) % len(valid_chunks)
                        chunk = valid_chunks[chunk_idx]
